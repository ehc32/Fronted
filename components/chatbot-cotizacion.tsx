...
import { Send, User, Phone, Bot, MessageCircle, ArrowLeft, Check } from "lucide-react"
...
const ChatbotCotizacion = ({ onBack }: ChatbotCotizacionProps) => {
  ...
  useEffect(() => {
    ...
    if (step === "greeting" && messages.length === 0) {
      addMessage(
        "bot",
...
⏱️ DURACIÓN DEL PROYECTO: 120 días calendario (4 meses)`
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-gray-900 to-black text-white px-6 py-5 shadow-xl border-b border-gray-800">
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          <div className="flex items-center gap-4">
            {onBack && (
              <button
                onClick={onBack}
                className="p-2.5 hover:bg-white/10 rounded-full transition-all duration-200 hover:scale-105 group"
              >
                <ArrowLeft className="w-5 h-5 group-hover:text-gray-300" />
              </button>
            )}
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-300 text-sm font-light">Sistema inteligente de cotizaciones</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-white/10 rounded-full backdrop-blur-sm">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
              <span className="text-sm text-gray-300 font-medium">En línea</span>
            </div>
            <div className="hidden lg:flex items-center gap-1 text-gray-300 hover:text-white transition-colors cursor-pointer">
              <span className="text-sm font-medium">Visitar Proyectos</span>
