...
const EnhancedChatbotCotizacion = ({ onBack }: ChatbotProps) => {
  ...
  const handleSpaceSelection = (option: QuestionOption) => {
    if (option.value === "terminar") {
      // User finished selecting spaces
    ...
    if (option.value === "ninguno") {
      ...
      const remainingSpaces = ADDITIONAL_SPACES.filter(
        (space) => !selectedSpaces.includes(space.value) && space.value !== "ninguno",
      )
  ...
  /**
   * Handle final action selection (download PDF or keep summary)
   */
  const handleFinalActions = (option: QuestionOption) => {
    if (option.action === "download") {
      ...
      setTimeout(() => {
        startNewQuotation()
      }, 1500) // Give a moment for the message to show
    } else {
      addMessage(
        "bot",
        ...
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-6">
            <Button
              ...
              onClick={() => window.open("https://www.saavearquitectos.com/", "_blank")} // Example URL for your site
